...
export interface FormProps extends Omit<ModalFormProps, 'onSubmit'> {
  ...
  copy?: boolean;
  id?: string;
  onSubmit?: (data: IProductReqData, options?: AfterSubmitFormOptions) => void;
...
const Form: React.FC<FormProps> = ({ edit, onSubmit, copy, defaultState, addInputs, ...props }) => {
  ...
  const [afterSubmitOptions, setAfterSubmitOptions] = useState<AfterSubmitFormOptions>({});
  ...
  function onValidSubmit(submitData: IProduct) {
    const omitPathArr: (keyof IProduct)[] =
      (formValues.priceType === 'DEFAULT' && ['countOut', 'subCountOut']) ||
      (formValues.priceType === 'COMMISSION' && ['countIn', 'subCountIn']) ||
      [];

    const trReqData = createDataForReq(submitData, omitPathArr);
    ...
    >
      <FlexBox className={'inputs'} flex={'1'} fillWidth maxHeight={'100%'} padding={'12px'} overflow={'auto'}>
        ...
        <InputLabel label={translate('comment')} direction={'vertical'}>
          <TextareaPrimary placeholder={translate('comment')} {...register('description')} />
        </InputLabel>
      </FlexBox>
    </ModalForm>
  );
